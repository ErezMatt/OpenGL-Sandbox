...
DiffuseLightScene::DiffuseLightScene(std::shared_ptr<SceneData> data)
    :_data(data)
{
    init();
}

void DiffuseLightScene::init()
{
...
void DiffuseLightScene::update(float dT)
{
    if (rotation >= 360)
        rotation -= 360;
    ++rotation;
...
void DiffuseLightScene::imGuiRender()
{
